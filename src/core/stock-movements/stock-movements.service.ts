...
import {
  ...
  CreateStockPurchaseOutput,
} from './dtos/create-stock-purchase.dto';
import {
...
export class StockMovementsService {
  ...
  async createStockAdjustment({
    ...
    try {
      ...
      const stockAdjustment = await this.stockAdjustments.findOne({ note });
      if (!stockAdjustment) {
  ...
  async editStockAdjustment({
    ...
    try {
      ...
      note && (stockAdjustment.note = note);
  ...
  async createStockPurchase({
    ...
    orderPurchaseId,
    quantity,
  }: CreateStockPurchaseInput): Promise<CreateStockPurchaseOutput> {
    try {
      const result = await this.productsService.findProductVariant({
        productVariantId,
      });
      ...
      const stockPurchase = await this.stockPurchases.findOne({
        where: { productVariant: result.productVariant },
      });
    } catch (error) {
      return {
        ok: false,
